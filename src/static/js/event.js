...
(function (definition) {
    ...
    var option = {};
    var eventlistener;
    THREE.onEvent = function(scene,camera, callback) {
        option.scene = scene || {};
        option.camera = camera || {};
        option.callback = callback;
        mainCanvas = document.getElementById("canvas-frame");
    }
    ...
    Object.assign(THREE.Object3D.prototype,{
        on: function(method,callback1,callback2) {
            if (EventListeners.hasOwnProperty(method)) {
                ...
                eventlistener = EventListeners[method];
    ...
    listenerList.click = function (targetList,camera) {
        ...
        function down(event) {
            ...
            Mouse.setFromCamera(new THREE.Vector2(( (event.clientX - mainCanvas.getBoundingClientRect().left) / mainCanvas.offsetWidth ) * 2 - 1,
                - ( (event.clientY - mainCanvas.getBoundingClientRect().top) / mainCanvas.offsetHeight ) * 2 + 1), camera);
            ...
            if (intersects.length > 0) { // mouse down trigger
                ...
                obj = getEventObj(targetList,targetObject);
                if(option.callback){
                    return option.callback(obj.object3d.name);
                }
            } else {
                Click = false;
            }
        ...
        mainCanvas.addEventListener('mouseup',up,false);
    }
